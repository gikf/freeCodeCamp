...
```js
assert.match(code, /<\s*select\s+[\s\S]*onChange\s*=\s*\{\s*(e|\(\s*e\s*\))\s*=>\s*(setPowerSource\s*\(\s*e\s*\.\s*target\s*\.\s*value\s*\)|\{\s*setPowerSource\s*\(\s*e\s*\.\s*target\s*\.\s*value\s*\)\s*;?\s*\})\s*\}/)
```
